...
if (!window.__tiqLoaded) {
  ...
  // Keyed by filename — prevents double buttons when Gmail renders
  // the same attachment in both collapsed + expanded card DOM nodes
  ...
  function handleSidebarMessage(event) {
    ...
    if (event.data.type === 'TIQ_FETCH_ATTACHMENT') {
      const { downloadUrl, filename, requestId } = event.data
      ...
      try {
        ...
        console.warn('[TIQ] sendMessage threw:', err.message)
        sidebarFrame?.contentWindow?.postMessage({
          type: 'TIQ_ATTACHMENT_DATA',
          filename,
          base64: '',
  ...
  function extractFilename(el) {
    ...
    if (tooltip && tooltip.includes('.')) {
      return tooltip.replace(/^(download|open|save)\s+/i, '').trim()
    }
  ...
  function extractDownloadUrl(el) {
    ...
    if (card) {
      ...
      // Extract message ID from end of Gmail URL — works for inbox, search,
      // and label views. The message ID is always the last path/hash segment
      // of 16+ hex characters. Using $ anchor avoids matching earlier segments
      // like search query words that also happen to be long alphanumeric strings.
  ...
  function injectButtons() {
    ...
    cards.forEach(card => {
      // Mark this node AND any nested [download-url] children as scanned.
      // Gmail nests [download-url] inside [data-legacy-attachment-id] — without
      // marking both, the child appears as a fresh unscanned card in the
      // fallback selector on the next injectButtons() call → double button.
      card.dataset.tiqScanned = '1'
      card.querySelectorAll('[download-url]').forEach(child => {
        child.dataset.tiqScanned = '1'
      })
      ...
      // KEY FIX: skip if we already injected a button for this filename.
      // Gmail renders the same attachment in two different DOM nodes
      // (collapsed thumbnail + expanded card) — the Set catches the second one.
      if (injectedFiles.has(filename)) return
  ...
  function init() {
    ...
    new MutationObserver(() => {
      if (location.href !== lastUrl) {
        ...
        // Reset everything on navigation so new email gets fresh buttons
