...
export default function CandidateDetailView({
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        const active = data.find((o: any) =>
          ['extended', 'accepted', 'joined'].includes(o.status)
        )
  ...
  const handleStageUpdate = async (newStage: string) => {
    if (!candidate) return
    if (!confirm(`Update stage to "${getStageLabel(newStage)}"?`)) return
    ...
    try {
      ...
      if (newStage === 'joined') {
        ...
        if (!joiningDate) { setUpdatingStage(false); return }
      ...
      // ── All other stage changes ────────────────────────────────────────────
        ...
        await supabase.from('candidate_timeline').insert([{
          ...
          activity_description: `Stage changed to: ${getStageLabel(newStage)}`,
          performed_by: currentUser?.id,
        }])
      ...
      loadCandidate(); loadTimeline(); loadOffers()
    } catch (error: any) {
  ...
  const handleMarkAsJoined = async () => {
    ...
    try {
      ...
      const { data: jobData } = await supabase
        ...
        .eq('id', candidate.job_id)
        .single()
      const clientData    = Array.isArray(jobData?.clients) ? jobData.clients[0] : jobData?.clients
      const guaranteeDays = clientData?.replacement_guarantee_days || 90
      const guaranteeEnds = new Date(joiningDate)
      guaranteeEnds.setDate(guaranteeEnds.getDate() + guaranteeDays)
      ...
      const { error: offerError } = await supabase
        .from('offers')
        .update({ status: 'joined', actual_joining_date: joiningDate, updated_at: new Date().toISOString() })
      ...
      await Promise.all([loadCandidate(), loadTimeline(), loadOffers()])
    } catch (error: any) {
  ...
  const handleRenege = async () => {
    ...
    try {
      ...
      if (activeOffer) {
        await supabase
          .from('offers')
          .update({ status: 'renege', updated_at: new Date().toISOString() })
          .eq('id', activeOffer.id)
      }
  ...
  const canCreateOffer = () => {
    ...
    const userData    = localStorage.getItem('user')
    const currentUser = userData ? JSON.parse(userData) : null
    if (userRole === 'recruiter') return candidate?.assigned_to === currentUser?.id
    return ['team_leader', 'sr_team_leader'].includes(userRole)
  }

  const canEditOffer = () => userRole === 'sr_team_leader' && !!activeOffer
  ...
  const renderOfferButtons = () => {
    ...
    if (activeOffer) {
      ...
      if (activeOffer.status === 'accepted') {
        return (
          <>
            <button onClick={handleMarkAsJoined} disabled={updatingStage}
            ...
            <button onClick={handleRenege} disabled={updatingStage}
        ...
        return (
          <>
            <div className="mt-6 px-6 py-2 bg-green-100 text-green-800 rounded-lg font-semibold inline-flex items-center gap-2">
              ...
              Joined on {activeOffer.actual_joining_date
                ? new Date(activeOffer.actual_joining_date).toLocaleDateString()
                : 'N/A'}
            </div>
            {candidate.guarantee_period_ends &&
              new Date() <= new Date(candidate.guarantee_period_ends) && (
                <button onClick={handleRenege} disabled={updatingStage}
                  className="mt-6 bg-red-600 text-white px-6 py-2 rounded-lg hover:bg-red-700 font-medium disabled:opacity-50">
                  Mark as Renege
  ...
  const handleOfferSuccess = async () => {
    try {
      ...
      const { error: stageError } = await supabase
        .from('candidates')
        .update({ current_stage: 'offer_extended', last_activity_date: new Date().toISOString() })
        .eq('id', candidateId)
  ...
  const handleViewResume = async () => {
    if (!candidate.resume_url) { alert('No resume uploaded for this candidate'); return }
    try {
      if (candidate.resume_url.includes('supabase')) {
        let filePath = ''
        if (candidate.resume_url.includes('/resumes/')) {
          filePath = decodeURIComponent(candidate.resume_url.split('/resumes/')[1] || '')
        } else if (candidate.resume_url.includes('/object/public/resumes/')) {
          filePath = decodeURIComponent(candidate.resume_url.split('/object/public/resumes/')[1] || '')
        }
        if (!filePath) throw new Error('Could not extract file path from URL')
        const { data, error } = await supabase.storage.from('resumes').createSignedUrl(filePath, 3600)
        if (error) throw error
  ...
  const getTimelineIcon = (type: string) => {
    const iconMap: Record<string, { color: string; symbol: string }> = {
      ...
      screening_rejected:  { color: 'bg-orange-500', symbol: '✗' },
      offer_extended:      { color: 'bg-green-500',  symbol: '$' },
      offer_rejected:      { color: 'bg-rose-500',   symbol: '✗' },
      candidate_joined:    { color: 'bg-teal-500',   symbol: 'J' },
      renege:              { color: 'bg-red-500',    symbol: 'R' },
      documentation:       { color: 'bg-cyan-500',   symbol: 'D' },
    }
  ...
  const isInterviewDisabled = () => {
    const disabledStages = [
      ...
      'documentation', 'joined', 'renege',
      'screening_rejected', 'interview_rejected',
    ]
    return disabledStages.includes(candidate?.current_stage || '')
  }
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div><label className="text-sm text-gray-500">Email</label><p className="font-medium">{candidate.email || 'N/A'}</p></div>
          <div><label className="text-sm text-gray-500">Phone</label><p className="font-medium">{candidate.phone}</p></div>
          <div><label className="text-sm text-gray-500">Source</label><p className="font-medium">{candidate.source_portal || 'N/A'}</p></div>
          <div><label className="text-sm text-gray-500">Location</label><p className="font-medium">{candidate.current_location || 'N/A'}</p></div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div><label className="text-sm text-gray-500">Current Company</label><p className="font-medium">{candidate.current_company || 'N/A'}</p></div>
          <div><label className="text-sm text-gray-500">Current Designation</label><p className="font-medium">{candidate.current_designation || 'N/A'}</p></div>
          <div><label className="text-sm text-gray-500">Total Experience</label><p className="font-medium">{candidate.total_experience || 0} years</p></div>
          <div><label className="text-sm text-gray-500">Relevant Experience</label><p className="font-medium">{candidate.relevant_experience || 0} years</p></div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div><label className="text-sm text-gray-500">Current CTC</label><p className="font-medium text-blue-600 text-xl">Rs.{candidate.current_ctc || 0}</p></div>
          <div><label className="text-sm text-gray-500">Expected CTC</label><p className="font-medium text-green-600 text-xl">Rs.{candidate.expected_ctc || 0}</p></div>
          <div><label className="text-sm text-gray-500">Notice Period</label><p className="font-medium">{candidate.notice_period || 0} days</p></div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div><label className="text-sm text-gray-500">Position</label><p className="font-medium">{candidate.jobs?.job_title || 'N/A'}</p></div>
          <div><label className="text-sm text-gray-500">Job Code</label><p className="font-medium">{candidate.jobs?.job_code || 'N/A'}</p></div>
          <div><label className="text-sm text-gray-500">Client</label><p className="font-medium">{candidate.jobs?.clients?.company_name || 'N/A'}</p></div>
          <div><label className="text-sm text-gray-500">Added By (Revenue Credit)</label><p className="font-medium text-green-600">{candidate.users?.full_name || 'N/A'}</p></div>
        </div>
      </div>
          ...
          <div className={showResumeUpload ? 'mt-4 border border-blue-200 rounded-lg p-4 bg-blue-50' : ''}>
            {showResumeUpload && (
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => setShowResumeUpload(false)}
                  className="text-blue-600 hover:text-blue-900 text-sm font-medium">✕ Cancel</button>
              </div>
            )}
            <ResumeUpload
            ...
            {candidate.job_id ? (
              <div className="p-6">
                <InterviewScheduler
                  ...
                  candidateName={candidate.full_name}
                  jobId={candidate.job_id}
                  onScheduled={() => { setShowScheduler(false); loadCandidate(); loadTimeline() }}
              ...
              <div className="p-6">
                ...
                <button onClick={() => setShowScheduler(false)} className="btn-primary w-full">Close</button>
              </div>
            )}
          </div>
        </div>
