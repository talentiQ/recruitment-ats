...
export default function MarkJoinedModal({ offer, onSuccess, onCancel }: MarkJoinedModalProps) {
  ...
  const getRevenueMonth = () => joiningDate.slice(0, 7)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error: offerError } = await supabase
        .from('offers')
        .update({ status: 'joined', actual_joining_date: joiningDate })
        .eq('id', offer.id)
      if (offerError) throw new Error(offerError.message)

      // 3. Create safety tracker
      await supabase.from('placement_safety_tracker').insert([{
        ...
        next_followup_date:    new Date(new Date(joiningDate).getTime() + 7 * 24 * 60 * 60 * 1000)
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
        ...
        <div className="mb-6">
          <label className="block text-sm font-medium text-gray-700 mb-2">Actual Joining Date *</label>
        ...
        {joiningDate && (
          <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
            <div className="flex justify-between">
              ...
              <span className="font-bold text-green-600">₹{calculateRevenue().toFixed(2)}</span>
            </div>
          </div>
        )}
