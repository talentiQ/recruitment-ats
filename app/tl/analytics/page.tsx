...
export default function TLAnalytics() {
  ...
  // ── STEP 1: Bootstrap team (TL + their direct recruiters) ─────────────────
  const bootstrapTeam = async (tl: any, range: { start: string; end: string }) => {
    ...
    try {
      ...
      // ── Single master fetch — all candidates active in range for this team ──
  ...
  // ── Per-member pipeline stats ──────────────────────────────────────────────
  const computeMemberProgress = async (
    allMembers: any[],
    tl: any,
    allCandidates: any[],
    ...
    const progress = await Promise.all(allMembers.map(async (member: any) => {
      ...
      const { count: jobs_assigned } = await supabase
        .from('job_recruiter_assignments').select('*', { count: 'exact', head: true }).eq('recruiter_id', member.id)
  ...
  const computeClientProgress = async (allCandidates: any[], range: { start: string; end: string }) => {
    ...
    const progress = await Promise.all(
      Array.from(clientMap.entries()).map(async ([clientId, clientName]) => {
        ...
        const counts = countFromCandidates(mine, range.start, range.end)
        return { client_id: clientId, client_name: clientName, new_jobs_added: new_jobs_added || 0, ...counts,
          conversion_rate: counts.cv_submitted > 0 ? Math.round((counts.joined / counts.cv_submitted) * 100) : 0 }
      })
    )
    setClientProgress(
  ...
  // ── Job-level pipeline stats ───────────────────────────────────────────────
  const computeJobProgress = async (
    allMemberIds: string[],
    teamId: string,
    allCandidates: any[],
    range: { start: string; end: string }
    ...
    const { data: assignedJobs } = await supabase
      .from('jobs').select('id, job_code, job_title, positions, clients!inner(company_name)')
  ...
  // ── Active candidate highlights (not date-filtered) ────────────────────────
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      const mH=['Member','Role','Jobs Assigned','CV Submitted','Interview Scheduled','Interview Completed','Interview Rejected','Offer Extended','Offer Accepted','Joined','Renege/Dropped','Conversion %']
      const mR=memberProgress.map(m=>[m.member_name,m.role==='team_leader'?'Team Leader':'Recruiter',m.jobs_assigned,m.cv_submitted,m.interview_scheduled,m.interview_completed,m.interview_rejected,m.offer_extended,m.offer_accepted,m.joined,m.renege_dropped,`${m.conversion_rate}%`])
      ...
      const wsM=XLSX.utils.aoa_to_sheet([mH,...mR]); wsM['!cols']=[{wch:24},{wch:14},...Array(10).fill({wch:20})]; XLSX.utils.book_append_sheet(wb,wsM,'Member Progress')
      ...
      XLSX.writeFile(wb,`TL_Analytics_${user?.full_name?.replace(/\s+/g,'_')}_${period.replace(/\s+/g,'_')}_${new Date().toISOString().slice(0,10)}.xlsx`)
