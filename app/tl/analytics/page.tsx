...
interface ClientProgress {
  client_id: string; client_name: string; new_jobs_added: number
  cv_submitted: number; interview_scheduled: number; interview_completed: number
  interview_rejected: number; offer_extended: number; offer_accepted: number
  joined: number; renege_dropped: number; conversion_rate: number
}
interface JobProgress {
  job_id: string; job_code: string; job_title: string; client_name: string; positions: number
  cv_submitted: number; interview_scheduled: number; interview_completed: number
  interview_rejected: number; offer_extended: number; offer_accepted: number
  joined: number; renege_dropped: number; filled_rate: number
}
interface CandidateHighlight {
  id: string; full_name: string; current_stage: string; job_title: string
  client_name: string; recruiter_name: string; last_activity_date: string; days_in_stage: number
}
...
function countFromCandidates(candidates: any[], start: string, end: string) {
  const startTs = new Date(start).getTime(), endTs = new Date(end).getTime()
  let cv_submitted=0,interview_scheduled=0,interview_completed=0,interview_rejected=0,offer_extended=0,offer_accepted=0,joined=0,renege_dropped=0
...
export default function TLAnalytics() {
  ...
  useEffect(()=>{
    ...
    if (parsedUser.role!=='team_leader'){alert('Access denied. Team Leader only.');router.push('/');return}
    setUser(parsedUser)
  },[])

  useEffect(()=>{ if(!user)return; bootstrapTeam(user,getDateRange()) },[user,selectedPeriod,customStartDate,customEndDate])
  ...
  const getDateRange = ()=>{
    ...
    if(selectedPeriod==='previous_day'){const y=new Date(today);y.setDate(y.getDate()-1);return{start:y.toISOString(),end:new Date(y.getFullYear(),y.getMonth(),y.getDate(),23,59,59).toISOString()}}
    if(selectedPeriod==='today')return{start:today.toISOString(),end:new Date(today.getFullYear(),today.getMonth(),today.getDate(),23,59,59).toISOString()}
    if(selectedPeriod==='week'){const ws=new Date(today);ws.setDate(ws.getDate()-7);return{start:ws.toISOString(),end:new Date(today.getFullYear(),today.getMonth(),today.getDate(),23,59,59).toISOString()}}
    if(selectedPeriod==='month')return{start:new Date(now.getFullYear(),now.getMonth(),1).toISOString(),end:new Date(now.getFullYear(),now.getMonth()+1,0,23,59,59).toISOString()}
  ...
  const bootstrapTeam = async (tl:any, range:{start:string;end:string})=>{
    setLoading(true)
    try{
      const {data:recruiters}=await supabase.from('users').select('id,full_name,role,reports_to').eq('reports_to',tl.id).eq('role','recruiter').eq('is_active',true)
      ...
      await Promise.all([
        ...
        computeMemberProgress(allMembers,tl,candidates,range),
        computeClientProgress(candidates,range),
        computeJobProgress(allMemberIds,tl.team_id,candidates,range),
  ...
  const computeTotals=async(candidates:any[],range:{start:string;end:string})=>{
    ...
    setTotals({total_cv_submitted:counts.cv_submitted,total_interview_scheduled:counts.interview_scheduled,total_interview_completed:counts.interview_completed,total_interview_rejected:counts.interview_rejected,total_offer_extended:counts.offer_extended,total_offer_accepted:counts.offer_accepted,total_joined:counts.joined,total_renege_dropped:counts.renege_dropped,overall_conversion:counts.cv_submitted>0?Math.round((counts.joined/counts.cv_submitted)*100):0})
  }

  const computeMemberProgress=async(allMembers:any[],tl:any,allCandidates:any[],range:{start:string;end:string})=>{
    const progress=await Promise.all(allMembers.map(async(member:any)=>{
      ...
      const {count:jobs_assigned}=await supabase.from('job_recruiter_assignments').select('*',{count:'exact',head:true}).eq('recruiter_id',member.id)
      const counts=countFromCandidates(mine,range.start,range.end)
      return{member_id:member.id,member_name:member.role==='team_leader'?`${member.full_name} (You)`:member.full_name,role:member.role,jobs_assigned:jobs_assigned||0,...counts,conversion_rate:counts.cv_submitted>0?Math.round((counts.joined/counts.cv_submitted)*100):0}
    }))
    setMemberProgress(progress.sort((a:any,b:any)=>{if(a.role==='team_leader')return -1;if(b.role==='team_leader')return 1;return b.cv_submitted-a.cv_submitted}))
  }
  ...
  const computeClientProgress=async(allCandidates:any[],range:{start:string;end:string})=>{
    ...
    const progress=await Promise.all(Array.from(clientMap.entries()).map(async([clientId,clientName])=>{
      ...
      const{count:new_jobs_added}=await supabase.from('jobs').select('*',{count:'exact',head:true}).eq('client_id',clientId).gte('created_at',range.start).lte('created_at',range.end)
      const counts=countFromCandidates(mine,range.start,range.end)
      return{client_id:clientId,client_name:clientName,new_jobs_added:new_jobs_added||0,...counts,conversion_rate:counts.cv_submitted>0?Math.round((counts.joined/counts.cv_submitted)*100):0}
    }))
    setClientProgress(progress.filter((c:any)=>c.new_jobs_added+c.cv_submitted+c.interview_scheduled+c.joined>0).sort((a:any,b:any)=>b.cv_submitted-a.cv_submitted))
  }

  const computeJobProgress=async(allMemberIds:string[],teamId:string,allCandidates:any[],range:{start:string;end:string})=>{
    const{data:assignedJobs}=await supabase.from('jobs').select('id,job_code,job_title,positions,clients!inner(company_name)').eq('assigned_team_id',teamId).eq('status','open')
    ...
    const progress=assignedJobs.map((job:any)=>{
      ...
      const counts=countFromCandidates(mine,range.start,range.end)
      return{job_id:job.id,job_code:job.job_code,job_title:job.job_title,client_name:job.clients.company_name,positions:job.positions,...counts,filled_rate:job.positions>0?Math.round((counts.joined/job.positions)*100):0}
    })
    setJobProgress(progress.filter((j:any)=>j.cv_submitted+j.interview_scheduled+j.joined>0).sort((a:any,b:any)=>b.cv_submitted-a.cv_submitted))
  }
  ...
  const loadHighlights=async(allMemberIds:string[])=>{
    ...
    const{data}=await supabase.from('candidates')
      ...
      .in('assigned_to',allMemberIds).in('current_stage',['interview_scheduled','interview_completed','offer_extended','offer_accepted','joined'])
      .order('last_activity_date',{ascending:false}).limit(50)
    if(!data)return
    const now=new Date()
    setHighlights(data.map((c:any)=>({id:c.id,full_name:c.full_name,current_stage:c.current_stage,job_title:c.jobs?.job_title||'N/A',client_name:c.jobs?.clients?.company_name||'N/A',recruiter_name:c.assigned_user?.full_name||'N/A',last_activity_date:c.last_activity_date,days_in_stage:Math.floor((now.getTime()-new Date(c.last_activity_date).getTime())/86400000)})))
  }
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      wsM.columns = [{ width: 24 }, { width: 14 }, ...Array(10).fill({ width: 20 })]
      wsM.addRow(['Member','Role','Jobs Assigned','CV Submitted','Interview Scheduled','Interview Completed','Interview Rejected','Offer Extended','Offer Accepted','Joined','Renege/Dropped','Conversion %'])
      memberProgress.forEach(m => wsM.addRow([m.member_name, m.role==='team_leader'?'Team Leader':'Recruiter', m.jobs_assigned, m.cv_submitted, m.interview_scheduled, m.interview_completed, m.interview_rejected, m.offer_extended, m.offer_accepted, m.joined, m.renege_dropped, `${m.conversion_rate}%`]))
      ...
      a.href       = url
      a.download   = `TL_Analytics_${user?.full_name?.replace(/\s+/g,'_')}_${period.replace(/\s+/g,'_')}_${new Date().toISOString().slice(0,10)}.xlsx`
  ...
  const getStageColor=(stage:string)=>{switch(stage){case 'interview_scheduled':return'bg-blue-100 text-blue-800 border-blue-300';case 'interview_completed':return'bg-purple-100 text-purple-800 border-purple-300';case 'offer_extended':return'bg-orange-100 text-orange-800 border-orange-300';case 'offer_accepted':return'bg-green-100 text-green-800 border-green-300';case 'joined':return'bg-emerald-100 text-emerald-800 border-emerald-300';default:return'bg-gray-100 text-gray-800 border-gray-300'}}
  const getStageLabel=(stage:string)=>stage.replace(/_/g,' ').replace(/\b\w/g,l=>l.toUpperCase())
  const groupedHighlights={interview_scheduled:highlights.filter(h=>h.current_stage==='interview_scheduled'),interview_completed:highlights.filter(h=>h.current_stage==='interview_completed'),offer_extended:highlights.filter(h=>h.current_stage==='offer_extended'),offer_accepted:highlights.filter(h=>h.current_stage==='offer_accepted'),joined:highlights.filter(h=>h.current_stage==='joined')}
  const recruitersCount=memberProgress.filter(m=>m.role==='recruiter').length

  const PipelineCell=({value,colKey}:{value:number;colKey:string})=>{const col=PIPELINE_COLS.find(c=>c.key===colKey)!;return<td className="px-4 py-3 text-center"><span className={`px-2 py-1 rounded font-semibold text-xs ${col.bg} ${col.text}`}>{value}</span></td>}
  const TotalCell=({value,colKey}:{value:number;colKey:string})=>{const col=PIPELINE_COLS.find(c=>c.key===colKey)!;return<td className="px-4 py-3 text-center"><span className={`px-2 py-1 rounded font-bold text-xs border-2 ${col.bg} ${col.text}`}>{value}</span></td>}
  const ConversionCell=({rate}:{rate:number})=>(<td className="px-4 py-3 text-center"><span className={`px-2 py-1 rounded font-bold text-xs ${rate>=20?'bg-green-100 text-green-800':rate>=10?'bg-yellow-100 text-yellow-800':'bg-red-100 text-red-800'}`}>{rate}%</span></td>)
    ...
    <DashboardLayout>
      <div className="max-w-7xl mx-auto space-y-6 pb-8">
        ...
        <div className="bg-gradient-to-r from-green-600 to-teal-600 rounded-lg p-6 text-white">
          ...
          <p className="text-green-100">{user?.full_name} · {recruitersCount} Recruiter{recruitersCount!==1?'s':''} · Including my own candidates</p>
        </div>
          ...
          <div className="flex flex-wrap gap-4 items-end">
            <div className="flex gap-2">
              {[{key:'previous_day',label:'Previous Day'},{key:'today',label:'Today'},{key:'week',label:'Last 7 Days'},{key:'month',label:'This Month'}].map(({key,label})=>(
                <button key={key} onClick={()=>setSelectedPeriod(key)} className={`px-4 py-2 rounded-lg font-medium transition ${selectedPeriod===key?'bg-green-600 text-white':'bg-gray-100 hover:bg-gray-200'}`}>{label}</button>
              ))}
            </div>
            <div className="flex gap-2 items-end">
              <div><label className="block text-xs text-gray-600 mb-1">From</label><input type="date" value={customStartDate} onChange={e=>{setCustomStartDate(e.target.value);setSelectedPeriod('custom')}} className="px-3 py-2 border rounded-lg text-sm"/></div>
              <div><label className="block text-xs text-gray-600 mb-1">To</label><input type="date" value={customEndDate} onChange={e=>{setCustomEndDate(e.target.value);setSelectedPeriod('custom')}} className="px-3 py-2 border rounded-lg text-sm"/></div>
            </div>
            <button onClick={exportToExcel} disabled={exporting} className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium ml-auto flex items-center gap-2 transition disabled:opacity-60">
          ...
          <div className="border-b border-gray-200">
            <div className="flex">
              {[{key:'member',label:`👤 Member Progress (${memberProgress.length})`},{key:'client',label:`🏢 Client Progress (${clientProgress.length})`},{key:'job',label:`💼 Job Progress (${jobProgress.length})`}].map(({key,label})=>(
                <button key={key} onClick={()=>setActiveTab(key as any)} className={`px-6 py-4 font-medium border-b-2 transition ${activeTab===key?'border-green-600 text-green-600':'border-transparent text-gray-600 hover:text-gray-900'}`}>{label}</button>
              ))}
            </div>
          </div>
          <div className="p-6">
            {activeTab==='member'&&(
              <div className="overflow-x-auto">
                {memberProgress.length===0?<div className="text-center py-8 text-gray-500">No team members found</div>:(
                      ...
                      {memberProgress.map(m=>(
                        <tr key={m.member_id} className={`hover:bg-gray-50 ${m.role==='team_leader'?'bg-green-50':''}`}>
                          <td className={`px-4 py-3 font-medium sticky left-0 ${m.role==='team_leader'?'bg-green-50':'bg-white'}`}>
                            {m.role==='recruiter'&&<span className="text-gray-300 mr-1">└</span>}{m.member_name}
                          </td>
                          <td className="px-4 py-3 text-center"><span className={`px-2 py-1 rounded-full text-xs font-medium ${m.role==='team_leader'?'bg-green-100 text-green-800':'bg-blue-100 text-blue-800'}`}>{m.role==='team_leader'?'TL (You)':'Recruiter'}</span></td>
            ...
            {activeTab==='client'&&(
              <div className="overflow-x-auto">
                {clientProgress.length===0?<div className="text-center py-8 text-gray-500">No client data for this period</div>:(
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50"><tr>
            ...
            {activeTab==='job'&&(
              <div className="overflow-x-auto">
                {jobProgress.length===0?<div className="text-center py-8 text-gray-500">No job data for this period</div>:(
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50"><tr>
                        ...
                        <tr key={job.job_id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-center">
                            <div className="flex items-center gap-2">
                              <div className="flex-1 bg-gray-200 rounded-full h-2"><div className={`h-2 rounded-full ${job.filled_rate>=80?'bg-green-500':job.filled_rate>=50?'bg-yellow-500':'bg-orange-500'}`} style={{width:`${Math.min(job.filled_rate,100)}%`}}/></div>
            ...
            {Object.entries(groupedHighlights).map(([stage,candidates])=>{
              ...
              return(
                <div key={stage} className="border-l-4 border-green-500 pl-4">
                  <h3 className="text-lg font-bold text-gray-900 mb-3"><span className={`px-3 py-1 rounded-full text-sm border-2 ${getStageColor(stage)}`}>{getStageLabel(stage)} ({candidates.length})</span></h3>
