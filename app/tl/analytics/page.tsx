...
export default function TLAnalytics() {
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      const mH=['Member','Role','Jobs Assigned','CV Submitted','Interview Scheduled','Interview Completed','Rejected','Offer Extended','Offer Accepted','Joined','Renege/Dropped','Conversion %']
      const mR=memberProgress.map(m=>[m.member_name,m.role==='team_leader'?'Team Leader':'Recruiter',m.jobs_assigned,m.cv_submitted,m.interview_scheduled,m.interview_completed,m.rejected,m.offer_extended,m.offer_accepted,m.joined,m.renege_dropped,`${m.conversion_rate}%`])
