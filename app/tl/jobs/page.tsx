  ...
  const loadJobs = async (userFromStorage: any) => {
    ...
    try {
      ...
      assignments?.forEach(a => {
        ...
        if (u) assignmentsByJob[a.job_id].push({ id: u.id, name: u.full_name, positions: a.positions_allocated })
      })
              ...
              <tbody>
                {filteredJobs.map(job => (
                  <tr key={job.id}>
                    ...
                    <td><span className="font-medium">{job.positions_filled || 0}/{job.positions}</span></td>
                    <td>
                      <button
