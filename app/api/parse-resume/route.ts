...
async function extractText(file: File): Promise<string> {
  ...
  // DOCX → mammoth (best accuracy)
  if (fileName.endsWith('.docx') || fileName.endsWith('.doc')) {
    const result = await mammoth.extractRawText({ buffer })
    return result.value || ''
  }

  // PDF → pdf-parse@1.1.1 (Vercel-compatible, simple function API)
  ...
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      if (resumeText.startsWith('%PDF')) {
        return NextResponse.json({ error: 'Send file via multipart/form-data, not raw bytes.' }, { status: 400 })
      }
    }
    ...
    console.log(`✅ confidence: ${(result.confidence * 100).toFixed(0)}%, skills: ${result.skills.length}, sector: ${result.sector}`)
