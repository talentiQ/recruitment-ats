...
function countFromCandidates(candidates: any[], start: string, end: string) {
  ...
  candidates.forEach((c: any) => {
    ...
    if (lastActivityTs >= startTs && lastActivityTs <= endTs) {
      ...
      if (['interview_rejected', 'screening_rejected', 'offer_rejected'].includes(stage)) interview_rejected++
...
export default function PipelineAnalytics() {
  ...
  const computeRecruiterProgress = async (allCandidates: any[], range: { start: string; end: string }) => {
    ...
    const progress = await Promise.all(recruiters.map(async (rec: any) => {
      ...
      return { recruiter_id: rec.id, recruiter_name: rec.full_name, jobs_assigned: jobs_assigned || 0,
  ...
  const computeClientProgress = async (allCandidates: any[], range: { start: string; end: string }) => {
    ...
    const progress = await Promise.all(clients.map(async (client: any) => {
      ...
      const counts = countFromCandidates(mine, range.start, range.end)
      return { client_id: client.id, client_name: client.company_name, new_jobs_added: new_jobs_added || 0,
        ...counts, conversion_rate: counts.cv_submitted > 0 ? Math.round((counts.joined / counts.cv_submitted) * 100) : 0 }
    }))
  ...
  // Management has 5 sheets (unique vs TL/Sr-TL):
  //   1. Overall Summary
  //   2. Recruiter Progress  ← header differs: no "Role" / "Reports To" columns
  //   3. Client Progress     ← "New Jobs Added" (not just "New Jobs")
  //   4. Job Progress
  //   5. Active Candidates   ← unique to management view
  const exportToExcel = async () => {
    ...
    try {
      ...
      const rT = recruiterProgress.reduce(
        (a,r) => ({ j:a.j+r.jobs_assigned, cv:a.cv+r.cv_submitted, is:a.is+r.interview_scheduled,
          ic:a.ic+r.interview_completed, rej:a.rej+r.interview_rejected, oe:a.oe+r.offer_extended,
          oa:a.oa+r.offer_accepted, jo:a.jo+r.joined, rd:a.rd+r.renege_dropped }),
        { j:0, cv:0, is:0, ic:0, rej:0, oe:0, oa:0, jo:0, rd:0 }
      )
      const wsRec = wb.addWorksheet('Recruiter Progress')
      wsRec.columns = [{ width: 22 }, ...Array(10).fill({ width: 20 })]
      wsRec.addRow(['Recruiter','Jobs Assigned','CV Submitted','Interview Scheduled','Interview Completed','Interview Rejected','Offer Extended','Offer Accepted','Joined','Renege/Dropped','Conversion %'])
      recruiterProgress.forEach(r => wsRec.addRow([
        r.recruiter_name, r.jobs_assigned, r.cv_submitted, r.interview_scheduled,
        r.interview_completed, r.interview_rejected, r.offer_extended,
        r.offer_accepted, r.joined, r.renege_dropped, `${r.conversion_rate}%`,
      ]))
      wsRec.addRow(['TOTAL', rT.j, rT.cv, rT.is, rT.ic, rT.rej, rT.oe, rT.oa, rT.jo, rT.rd,
        rT.cv > 0 ? `${Math.round(rT.jo / rT.cv * 100)}%` : '0%'])

      // ── Sheet 3: Client Progress ──
      const cT = clientProgress.reduce(
        (a,c) => ({ nj:a.nj+c.new_jobs_added, cv:a.cv+c.cv_submitted, is:a.is+c.interview_scheduled,
          ic:a.ic+c.interview_completed, rej:a.rej+c.interview_rejected, oe:a.oe+c.offer_extended,
          oa:a.oa+c.offer_accepted, jo:a.jo+c.joined, rd:a.rd+c.renege_dropped }),
        { nj:0, cv:0, is:0, ic:0, rej:0, oe:0, oa:0, jo:0, rd:0 }
      )
      const wsCl = wb.addWorksheet('Client Progress')
      wsCl.columns = [{ width: 28 }, ...Array(10).fill({ width: 20 })]
      wsCl.addRow(['Client','New Jobs Added','CV Submitted','Interview Scheduled','Interview Completed','Interview Rejected','Offer Extended','Offer Accepted','Joined','Renege/Dropped','Conversion %'])
      clientProgress.forEach(c => wsCl.addRow([
        c.client_name, c.new_jobs_added, c.cv_submitted, c.interview_scheduled,
        c.interview_completed, c.interview_rejected, c.offer_extended,
        c.offer_accepted, c.joined, c.renege_dropped, `${c.conversion_rate}%`,
      ]))
      wsCl.addRow(['TOTAL', cT.nj, cT.cv, cT.is, cT.ic, cT.rej, cT.oe, cT.oa, cT.jo, cT.rd,
        cT.cv > 0 ? `${Math.round(cT.jo / cT.cv * 100)}%` : '0%'])

      // ── Sheet 4: Job Progress ──
      const jT = jobProgress.reduce(
        (a,j) => ({ pos:a.pos+j.positions, cv:a.cv+j.cv_submitted, is:a.is+j.interview_scheduled,
          ic:a.ic+j.interview_completed, rej:a.rej+j.interview_rejected, oe:a.oe+j.offer_extended,
          oa:a.oa+j.offer_accepted, jo:a.jo+j.joined, rd:a.rd+j.renege_dropped }),
        { pos:0, cv:0, is:0, ic:0, rej:0, oe:0, oa:0, jo:0, rd:0 }
      )
      const wsJb = wb.addWorksheet('Job Progress')
      wsJb.columns = [{ width: 12 }, { width: 28 }, { width: 24 }, ...Array(10).fill({ width: 20 })]
      wsJb.addRow(['Job Code','Job Title','Client','Positions','CV Submitted','Interview Scheduled','Interview Completed','Interview Rejected','Offer Extended','Offer Accepted','Joined','Renege/Dropped','Filled %'])
      jobProgress.forEach(j => wsJb.addRow([
        j.job_code, j.job_title, j.client_name, j.positions, j.cv_submitted,
        j.interview_scheduled, j.interview_completed, j.interview_rejected,
        j.offer_extended, j.offer_accepted, j.joined, j.renege_dropped, `${j.filled_rate}%`,
      ]))
      wsJb.addRow(['TOTAL', '', '', jT.pos, jT.cv, jT.is, jT.ic, jT.rej, jT.oe, jT.oa, jT.jo, jT.rd,
        jT.pos > 0 ? `${Math.round(jT.jo / jT.pos * 100)}%` : '0%'])

      // ── Sheet 5: Active Candidates (management-only sheet) ──
      const wsHl = wb.addWorksheet('Active Candidates')
      wsHl.columns = [{ width: 24 }, { width: 22 }, { width: 28 }, { width: 24 }, { width: 20 }, { width: 14 }, { width: 16 }]
      wsHl.addRow(['Candidate Name','Current Stage','Job Title','Client','Recruiter','Days in Stage','Last Activity'])
      highlights.forEach(h => wsHl.addRow([
        h.full_name,
      ...
      a.href       = url
      a.download   = `Pipeline_Analytics_${period.replace(/\s+/g,'_').replace(/\//g,'-')}_${new Date().toISOString().slice(0,10)}.xlsx`
