...
const STAGE_COLORS: Record<string, string> = {
  rejected: 'bg-red-100 text-red-800', hold: 'bg-gray-100 text-gray-700',
  on_hold: 'bg-orange-100 text-orange-800', interview_completed: 'bg-blue-100 text-blue-800',
  offer_extended: 'bg-indigo-100 text-indigo-800', offer_accepted: 'bg-violet-100 text-violet-800',
  ...
  screening: 'bg-yellow-100 text-yellow-800', interview_scheduled: 'bg-blue-100 text-blue-700',
  shortlisted: 'bg-cyan-100 text-cyan-800', offer_sent: 'bg-purple-100 text-purple-800',
...
// ─── More Interviews In Progress Badge ───────────────────────────────────────
const MIPBadge = () => (
...
export default function ManagementInterviewsPage() {
  ...
  const loadAll = async (currentUser?: any) => {
    ...
    try {
      ...
      const uMap: Record<string, any> = { [me.id]: me }
      allTeamMembers.forEach((m: any) => { uMap[m.id] = m })

      const resolveTeam = (userId: string): string => {
        const u = uMap[userId]
        if (!u || u.id === me.id) return me.full_name
        if (!u.reports_to || u.reports_to === me.id) return me.full_name
        const parent = uMap[u.reports_to]
        return parent ? parent.full_name + "'s TL" : me.full_name
      }
                          ...
                          {ivs.sort((a, b) => (a.interview_time || '').localeCompare(b.interview_time || '')).map(iv => (
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0" onClick={() => setSelectedInterview(iv)}>
                                ...
                                {/* Cancel / reschedule reasons inline */}
                                {iv.cancel_reason && (
                              ...
                              <div className="text-right text-sm min-w-[140px] flex flex-col gap-2">
                                ...
                                {/* Quick action buttons — only for non-cancelled/completed */}
                                {!['cancelled', 'completed', 'no_show', 'rejected'].includes(iv.status) && tab === 'active' && (
                                  <button
                                    onClick={e => { e.stopPropagation(); setSchedulerInterview(iv) }}
                                    className="text-xs px-2 py-1 border border-blue-200 text-blue-600 rounded-lg hover:bg-blue-50 font-medium transition self-end">
                                    ⚙️ Manage
                                  </button>
                                )}
                              </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-5 space-y-3">
              ...
              <div className="flex gap-2 pt-1">
                <button
                  onClick={() => { router.push(`/sr-tl/candidates/${selectedInterview.candidate_id}`); setSelectedInterview(null) }}
                ...
                {!['cancelled', 'completed', 'no_show', 'rejected'].includes(selectedInterview.status) && (
                  <button
                    ...
                    className="px-4 py-2.5 bg-blue-50 border border-blue-200 text-blue-700 rounded-lg text-sm font-semibold hover:bg-blue-100 transition">
                    ⚙️ Manage
                  </button>
                )}
