    ...
    <DashboardLayout>
      <div style={{ maxWidth: 1100, margin: '0 auto', paddingBottom: 60, fontFamily: "'Inter','Segoe UI',sans-serif" }}>
        ...
        {tab === 'holidays' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 10, padding: '12px 16px', fontSize: 13, color: '#2563eb' }}>
              ℹ️ Holidays are managed by Management. All holidays count as Present for your team.
            </div>
            ...
            {holidayLoading ? (
              ...
              <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #e5e7eb', overflow: 'hidden' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '130px 1fr 130px 110px', gap: 4, padding: '10px 16px', background: 'linear-gradient(135deg,#0f172a,#1e3a5f)', fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.7)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  <div>Date</div><div>Holiday</div><div>Day</div><div>Type</div>
                </div>
                {holidays.map((h, i) => {
                  const d = new Date(h.date + 'T00:00:00') // avoid UTC shift
                  const isPast = new Date(h.date) < new Date(new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Kolkata' }))
                  return (
                    <div key={h.id} style={{ display: 'grid', gridTemplateColumns: '130px 1fr 130px 110px', gap: 4, alignItems: 'center', padding: '14px 16px', background: isPast ? '#fafafa' : '#fff', borderBottom: i < holidays.length - 1 ? '1px solid #f1f5f9' : 'none', opacity: isPast ? 0.7 : 1 }}>
                      <div style={{ fontWeight: 700, fontSize: 14, color: '#1e293b' }}>{h.date}</div>
                      <div style={{ fontWeight: 600, fontSize: 14, color: '#1e293b' }}>
                        {h.name}
                        {!isPast && (
                          <span style={{ marginLeft: 8, fontSize: 10, background: '#f5f3ff', color: '#6d28d9', padding: '1px 7px', borderRadius: 100, fontWeight: 600 }}>Upcoming</span>
                        )}
                      </div>
