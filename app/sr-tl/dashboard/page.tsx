  ...
  const loadDashboard = async (user: any) => {
    ...
    try {
      ...
      if (allTeamMembers.length > 0) {
        ...
        // Active pipeline stages — all stages that are not terminal/rejected
        const ACTIVE_PIPELINE_STAGES = [
          'sourced', 'screening', 'interview_scheduled', 'interview_completed',
          'documentation', 'offer_extended', 'offer_accepted',
        ]
        ...
        const performanceData = await Promise.all(
          allTeamMembers.map(async (member: any) => {
            ...
            // Active pipeline = all non-terminal stages (excl. joined, all rejections, renege, on_hold)
            const { count: pipelineCount } = await supabase
              .from('candidates')
              .select('*', { count: 'exact', head: true })
              .eq('assigned_to', member.id)
              .in('current_stage', ACTIVE_PIPELINE_STAGES)
                  ...
                  {memberPerformance.map(member => (
                    <tr key={member.user_id}>
                      ...
                      <td className="text-center">
                        ...
                        }`}>{member.monthly_achievement}%</span>
                      </td>
                      ...
                      <td className="text-center">
                        <span className={`px-3 py-1 rounded-full text-sm font-bold ${
                          ...
                          'bg-red-100 text-red-800'
                        }`}>{member.quarterly_achievement}%</span>
                      </td>
                      ...
                      <td className="text-center">
                        <span className={`px-3 py-1 rounded-full text-sm font-bold ${
                          ...
                          'bg-red-100 text-red-800'
                        }`}>{member.annual_achievement}%</span>
                      </td>
                    </tr>
                  ))}
                  {/* Team Total Row */}
          ...
          {memberPerformance.length === 0 ? (
            <div className="text-center py-16 bg-gray-50 rounded-lg">
              ...
              <div className="flex gap-4 justify-center">
                ...
                <button onClick={() => router.push('/sr-tl/jobs')}
                        ...
                        <tr key={member.user_id} className="hover:bg-gray-50">
                          ...
                          <td>
                            <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                              member.role === 'team_leader' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
                            }`}>
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            >
              ...
              <div className="text-sm text-blue-700">{teamStats.total_candidates || 0} total</div>
            </button>
            <button
            ...
            >
              ...
              <div className="text-sm text-green-700">{teamStats.active_jobs || 0} active</div>
            </button>
            <button
              onClick={() => router.push('/sr-tl/team-leaders')}
            ...
            >
              ...
              <div className="text-sm text-purple-700">{teamStats.total_tls || 0} leaders</div>
            </button>
            <button
